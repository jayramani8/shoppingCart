...
const AddToCart = (props) => {
  if (props.cartData.length) {
    ...
    props.cartData.forEach((object) => {
      object.qty = parseInt(1);
    });
    // console.log(cartData);
    let TotalPrice = cartData.reduce((n, { price }) => n + price, 0);
    let TotalItem = cartData.reduce((n, { qty }) => n + qty, 0);
    ...
    const onCheckOut = () => {
      const productId = cartData.map((a) => a.id);
      const productQty = cartData.map((a) => a.qty);
      if (toalItem > 0) {
        Router.push({
